...
ParseResultCode expected_number_error(const char *field_name) {
    ...
    return error(msg, PARSE_INVALID_FIELD);
}

ParseResultCode expected_existent_field(const char *field_name) {
    char msg[50];
    sprintf(msg, "Field \"%s\" not found", field_name);
    return error(msg, PARSE_INVALID_FIELD);
}
...
ParseResultCode get_number_field(cJSON *parent_json, const char *field_name, int *result) {
    ...
    if (field_json == NULL) {
        return expected_existent_field(field_name);
    }
